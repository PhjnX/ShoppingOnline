  ...
  render() {
    const customers = this.state.customers.map((item) => {
      return (
        <tr
          key={item._id}
          className=" bg-white border-b bordesr-x transition-all hover:dark:bg-orange-200 hover:dark:border-orange-200 hover:dark:text-white text-center"
          onClick={() => this.trCustomerClick(item)}
        >
          <td
            data-lable="ID"
            className="break-words text-[16px] shadow xl:shadow-none px-6 py-3 cursor-pointer text-gray-600"
          >
            {item._id}
          </td>
          <td
            data-lable="UserName"
            className="text-[16px] px-6 py-3 cursor-pointer text-gray-600"
          >
            {item.username}
          </td>
          ...
          <td
            data-lable="Name"
            className="text-[16px] shadow xl:shadow-none px-6 py-3 cursor-pointer text-gray-600"
          >
            {item.name}
          </td>
          <td
    ...
    const orders = this.state.orders.map((item) => {
      return (
        ...
        >
          ...
          <td
            data-lable="Status"
            className="text-[16px] shadow xl:shadow-none px-6 py-3 cursor-pointer text-gray-900"
          >
            {item.status}
          </td>
        </tr>
      ...
      var items = this.state.order.items.map((item, index) => {
        return (
          <tr
            ...
            className="text-[16px]  bg-white border-b border-x transition-all hover:dark:bg-orange-200 hover:dark:border-orange-200 hover:dark:text-white text-center"
          >
            <td
              data-lable="NO."
              className="text-[16px] shadow xl:shadow-none px-6 py-3 cursor-pointer text-gray-600"
            ...
            <td
              data-lable="ID"
              className="text-[16px] break-words shadow xl:shadow-none px-6 py-3 cursor-pointer text-gray-600"
            >
              {item.product._id}
            </td>
            <td
            ...
            >
              <img
                ...
                className="ml-auto w-[50px] h-[70px] object-cover"
                alt=""
              />
            </td>
            <td
      ...
      <div>
        ...
        {this.state.orders.length > 0 ? (
          <div className="container-70 py-8">
            <div className="text-center mb-5">
              <Typography className="text-[20px] font-bold font-montserrat">
                DANH SÁCH TRẠNG THÁI ĐẶT/MUA HÀNG
              </Typography>
            </div>
            <table
              className="w-full text-sm text-left text-gray-500 dark:text-gray-400 table"
